...
const loginAdmin = async (req, res) => {
    ...
    try {
        ...
        if (user && await comparePassword(password, user.password)) {
            ...
            res.clearCookie('bloodBankTokenAdmin', cookieOptions);
            res.clearCookie('bloodBankTokenDonor', cookieOptions);
            res.clearCookie('bloodBankTokenOrganization', cookieOptions);
            res.cookie('bloodBankTokenAdmin', token, cookieOptions);
...
const loginDonor = async (req, res) => {
    ...
    if (!nic || !password) {
        return res.status(400).json({ error: 'NIC and password must be provided' });
    }
    ...
    try {
        ...
        if (user && await comparePassword(password, user.password)) {
            ...
            res.clearCookie('bloodBankTokenDonor', cookieOptions);
            res.clearCookie('bloodBankTokenOrganization', cookieOptions);
            res.cookie('bloodBankTokenDonor', token, cookieOptions);
            console.log("User authenticated successfully");
            return res.status(200).json(user);
        } else {
            console.log("User not found or invalid password");
...
const loginOrganization = async (req, res) => {
    ...
    if (!registrationNumber || !password) {
        return res.status(400).json({ error: 'Registration number and password must be provided' });
    }
    ...
    try {
        ...
        if (user && await comparePassword(password, user.password)) {
            ...
            res.clearCookie('bloodBankTokenDonor', cookieOptions);
            res.clearCookie('bloodBankTokenOrganization', cookieOptions);
            res.cookie('bloodBankTokenOrganization', token, cookieOptions);
            console.log("User authenticated successfully");
            return res.status(200).json(user);
        } else {
            console.log("User not found or invalid password");
